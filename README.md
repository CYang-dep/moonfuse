...
- 已完成：协议枚举、固定头解析、错误码、属性和目录项模型、回复模型、Filesystem trait、请求分类、句柄模型。
- 验证：`moon check`、`moon test`、`moon info`、`moon fmt`；CI 在 Ubuntu 上重复这些检查。
- 设计边界：协议核心不依赖具体文件系统，便于接入内存树、归档视图和动态配置等多类实现。
